                ...
                <div class="card">
                    ...
                    <div class="card-body table-responsive p-0">
                        <table class="table table-hover">
                            ...
                            <tr v-for="(disease , index) in diseases.data" :key="index" class="small">

                                <td>{{disease.full_name}}</td>



                                <td>{{disease.birth_at|myDate2}}</td>
                                <td>{{disease.diagnosis_at|myDate2}}</td>
                                <td>
                                    {{disease.region_point.region_center.region_county.name}} -
                                    {{disease.region_point.region_center.name}} -
                                    {{disease.region_point.name}}
                                </td>


                                <td>
                                    <a href="#" @click="edit(disease)" >
                                        <i class="fa fa-edit blue"></i>
                                    </a>
                                    /
                                    <a href="#" @click="deleteValue(disease.id)">
                    ...
                    <div class="card-footer">
                        ...
                        <pagination :data="diseases" size="small" :limit="8" @pagination-change-page="getResults"></pagination>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            getResults(page = 1) {
                axios.get('api/cd/disease/PaginateByCounty?page=' + page)
                    .then(response => {
            ...
            updateValue() {
                ...
                this.form.put('api/cd/disease/' + this.form.id)
                    .then(() => {
            ...
            deleteValue(id) {
                swal.fire({
                    ...
                    if (result.value) {
                        this.form.delete('/api/cd/disease/' + id).then(() => {
                            swal.fire(
                                'پاک شد',
                                'با موفقیت حذف شد',
                                'success'
            ...
            loadValues() {
                if (this.$gate.isAdminOrGroup_admin()) {
                    this.$Progress.start();
                    axios.get("api/cd/disease/PaginateByCounty").then(({data}) => (this.diseases = data)).then(() => {
                        this.$Progress.finish();
                    }).catch(() => {
                        this.$Progress.fail();
                        toast.fire({
        ...
        created() {
            ...
            Fire.$on('searching', () => {
                ...
                axios.get('api/cd/disease/findValue?q=' + query)
                    .then((data) => {
