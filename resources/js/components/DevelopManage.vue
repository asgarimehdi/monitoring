<template>
    ...
    <div class="container-fluid">
        <div id="isAdmin" v-if="$gate.isAdmin()||($gate.isGroup_admin() && $gate.isGostaresh())">
            <div class="row">
                ...
                <div class="modal-dialog modal-dialog-centered" role="document">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title" v-show="editmode">ویرایش جمعیت</h5>
                            <h5 class="modal-title" v-show="!editmode">ثبت جمعیت جدید</h5>

                        </div>
                        <form @submit.prevent="editmode ? updatePoint() : createPoint()">
                            <div class="modal-body">
                                <div class="form-group input-group">
                                    <input v-model="form.population" type="text" name="population"
                                           placeholder="population"
                                           class="form-control" :class="{ 'is-invalid': form.errors.has('population') }">
                                    <div class="input-group-prepend">
                                        <span class="input-group-text pl-5">جمعیت</span>
                                    </div>
                                    <has-error :form.population="form" field="population"></has-error>
                                </div>
        ...
        <div v-if="!($gate.isAdmin()||($gate.isGroup_admin() && $gate.isGostaresh()))">
            <not-found></not-found>
        </div>
        ...
        methods: {
            ...
            editmodalPoint() {
                if (this.pointById.id) {
                    ...
                    this.form.reset();
                    $('#modalPoint').modal('show');
                    this.form.fill(this.pointById.develop);
                } else {
                    toast.fire({
                        type: 'error',
            ...
            updatePoint() {
                ...
                this.form.put('api/develop/' + this.form.id)
                    .then(() => {
                        this.$emit('PointTableChanged');
                        $('#modalPoint').modal('hide');
                        toast.fire({
